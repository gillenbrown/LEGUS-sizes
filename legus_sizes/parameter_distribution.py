...
#
# ======================================================================================
catalogs = [table.Table.read(item, format="ascii.ecsv") for item in sys.argv[1:]]
...
axs[0].hist(big_catalog["scale_radius_pixels"], bin_size=1)
axs[0].add_labels("Scale Radius [pixels]", "Number of Clusters")
axs[0].set_limits(0)

axs[1].hist(big_catalog["axis_ratio"], bin_size=0.05)
axs[1].add_labels("Axis ratio", "Number of Clusters")
axs[1].set_limits(0, 1)

axs[2].hist(big_catalog["power_law_slope"], bin_size=0.1)
axs[2].add_labels("$\eta$ (Power Law Slope)", "Number of Clusters")
axs[2].set_limits(0, 10)
axs[2].axvline(1, ls=":")

axs[3].hist(big_catalog["effective_radius_pc_rmax_100_pc"], bin_size=1)
axs[3].add_labels("Effective Radius (pc)", "Number of Clusters")
axs[3].set_limits(0)
